...
export default function ObservabilityDashboard({ initialStats }) {
  ...
  return (
    <div className="module-container flex flex-col justify-between h-full" style={{ display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto' }}>
      ...
      {/* DASHBOARD HEADER */}
      <div className="flex justify-between items-center mb-4" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <div className="flex flex-col" style={{ display: 'flex', flexDirection: 'column' }}>
          <h3 className="section-title text-xl font-bold" style={{ color: 'var(--text-primary)', fontSize: '1.25rem', margin: 0 }}>
            PharmaCorp Advisory Prep · Context Pipeline Run #{runNumber}
          </h3>
          <span className="text-[10px] font-mono text-slate-500 mt-1" style={{ fontFamily: 'JetBrains Mono', fontSize: '0.65rem', color: 'var(--text-muted)' }}>
        ...
        <div className="flex gap-2.5" style={{ display: 'flex', gap: '10px' }}>
          ...
          <button
            ...
            style={{
              padding: '8px 14px', fontSize: '0.72rem', fontWeight: 700, border: '1px solid var(--panel-border)', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s',
              backgroundColor: 'rgba(13, 16, 26, 0.2)', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '6px'
            }}
          >
      ...
      <div className="flex gap-2 mb-5" style={{ display: 'flex', gap: '8px', marginBottom: '16px', borderBottom: '1px solid var(--panel-border)', paddingBottom: '8px' }}>
        <button
          ...
          style={{
            ...
            borderColor: dashTab === 'executive' ? 'rgba(99, 102, 241, 0.3)' : 'var(--panel-border)',
            backgroundColor: dashTab === 'executive' ? 'rgba(99, 102, 241, 0.05)' : 'rgba(5, 7, 12, 0.3)',
            color: dashTab === 'executive' ? '#a5b4fc' : 'var(--text-muted)'
          }}
        >
          📊 Executive Summary
        </button>
        <button
          onClick={() => setDashTab('deepdive')}
          className={`px-4 py-2 text-xs font-bold rounded-lg border transition-all cursor-pointer ${
            dashTab === 'deepdive'
              ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400 font-bold'
              : 'bg-slate-950/20 border-slate-800/80 text-slate-500 hover:text-slate-400'
          ...
          style={{
            padding: '6px 14px', fontSize: '0.72rem', fontWeight: 700, border: '1px solid', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s',
            borderColor: dashTab === 'deepdive' ? 'rgba(99, 102, 241, 0.3)' : 'var(--panel-border)',
            backgroundColor: dashTab === 'deepdive' ? 'rgba(99, 102, 241, 0.05)' : 'rgba(5, 7, 12, 0.3)',
            color: dashTab === 'deepdive' ? '#a5b4fc' : 'var(--text-muted)'
          }}
        >
      ...
      {dashTab === 'executive' ? (
        <div className="flex flex-col gap-6 animate-fadeIn" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          
          {/* CLOSE THE LOOP OPTIMIZATION DELTA ANNOTATION */}
          {closedLoopApplied && (
            <div className="p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 text-slate-300 animate-fadeIn" style={{
              padding: '14px 18px',
              borderRadius: '12px',
          ...
          <div className="grid-layout" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          ...
          {/* Quality by Run Bar chart */}
          <div className="glass-panel" style={{ padding: '20px', height: '260px' }}>
            <span className="text-xs font-semibold text-slate-300 block mb-4" style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600, display: 'block', marginBottom: '16px' }}>Context Quality Score by Run</span>
            <div style={{ width: '100%', height: '180px' }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={runsData}>
                  ...
                  <RechartsTooltip contentStyle={{ backgroundColor: 'var(--bg-dark)', border: '1px solid var(--panel-border)', fontSize: '10px' }} />
          ...
          <div className="border border-indigo-500/20 bg-indigo-500/5 rounded-xl p-6 text-center shadow-lg"
            style={{
          ...
          >
            <blockquote className="text-lg italic font-semibold text-slate-200 mb-4" style={{ fontSize: '1.15rem', color: 'var(--text-primary)', fontStyle: 'italic', fontWeight: 600, margin: '0 0 16px 0' }}>
        ...
        <div className="flex flex-col gap-6 animate-fadeIn" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* CHARTS ROW 1 (Freshness Decay) */}
          <div className="glass-panel" style={{ padding: '20px', height: '260px' }}>
            <span className="text-xs font-semibold text-slate-300 block mb-4" style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600, display: 'block', marginBottom: '16px' }}>Freshness Decay Over Time (7-Day Metric)</span>
            <div style={{ width: '100%', height: '180px' }}>
              <ResponsiveContainer width="100%" height="100%">
            ...
            <span className="text-[8px] font-mono text-slate-500 block text-right mt-1" style={{ fontSize: '0.58rem', fontFamily: 'JetBrains Mono', color: 'var(--text-muted)', textAlign: 'right', display: 'block', marginTop: '4px' }}>
          ...
          {/* CHARTS ROW 2 (Radar and Timeline) */}
          <div className="grid-layout mb-6" style={{ display: 'grid', gridTemplateColumns: '0.9fr 1.1fr', gap: '20px' }}>
            
            {/* Left: Health Radar Chart */}
            <div className="glass-panel" style={{ padding: '20px', height: '320px' }}>
              <span className="text-xs font-semibold text-slate-300 block mb-4" style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600, display: 'block', marginBottom: '16px' }}>Pipeline Health</span>
            ...
            <div className="glass-panel flex flex-col" style={{ padding: '20px', height: '320px', display: 'flex', flexDirection: 'column' }}>
              <span className="text-xs font-semibold text-slate-300 block mb-4" style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600, display: 'block', marginBottom: '16px' }}>Context Decisions Log</span>
              ...
              <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-3" style={{ flexGrow: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {decisions.map((log, index) => (
                  <div key={index} className="flex items-start gap-3 text-xs leading-normal" style={{ display: 'flex', gap: '12px', fontSize: '0.78rem', lineHeight: '1.4' }}>
                    <span className="font-mono text-slate-500 text-[10px] mt-0.5" style={{ fontFamily: 'JetBrains Mono', fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '2px' }}>
                      {log.time}
                    </span>
